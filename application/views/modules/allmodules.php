                ...
                <tbody>
                  <?php $n=0; if($$table_name == 'No data' || $$table_name == 'Table not exists'){}else{
                    foreach ($$table_name as $row) { $n++;
                      $rnd=rand(1, 99999999);
                      $hash='sqxn-'.$table_name.'-'.$row['id'].'-'.$rnd.'-'.$controller.'-record Deleted!';
      ...
      <?php echo form_open($controller.'/add'); ?>
        <div class="modal-body"> 
          ...
          <div class="form-group">
            ...
            <input type="text" id="slug" name="slug" class="form-control" placeholder="enter module name">
          </div>

          <div class="form-group" id="next_btn">
            <button type="button" class="btn btn-success pull-right" onclick="saveAndNext()">Next</button>
            <div class="clearfix"></div>
          </div>

          <input type="hidden" id="field_count" name="field_count" value="0">
          <div id="fields_data"></div>
...
<script src="<?php echo base_url('assets/theme/'); ?>dist/js/module_generator.js"></script>
<script>
